...
wy = read.table ("~/Desktop/PDA/W_Y.txtratio_peptide_None.tsv", head=T)
wa = read.table ("~/Desktop/PDA/W_A.txtratio_peptide_None.tsv", head=T)
wg = read.table ("~/Desktop/PDA/W_G.txtratio_peptide_None.tsv", head=T)
wh = read.table ("~/Desktop/PDA/W_H.txtratio_peptide_None.tsv", head=T)
wc = read.table ("~/Desktop/PDA/W_C.txtratio_peptide_None.tsv", head=T)
wd = read.table ("~/Desktop/PDA/W_D.txtratio_peptide_None.tsv", head=T)
we = read.table ("~/Desktop/PDA/W_E.txtratio_peptide_None.tsv", head=T)
wi = read.table ("~/Desktop/PDA/W_I.txtratio_peptide_None.tsv", head=T)
wl = read.table ("~/Desktop/PDA/W_L.txtratio_peptide_None.tsv", head=T)
wm = read.table ("~/Desktop/PDA/W_M.txtratio_peptide_None.tsv", head=T)
wn = read.table ("~/Desktop/PDA/W_N.txtratio_peptide_None.tsv", head=T)
wp = read.table ("~/Desktop/PDA/W_P.txtratio_peptide_None.tsv", head=T)
wq = read.table ("~/Desktop/PDA/W_Q.txtratio_peptide_None.tsv", head=T)
ws = read.table ("~/Desktop/PDA/W_S.txtratio_peptide_None.tsv", head=T)
wt = read.table ("~/Desktop/PDA/W_T.txtratio_peptide_None.tsv", head=T)
wv = read.table ("~/Desktop/PDA/W_V.txtratio_peptide_None.tsv", head=T)

#selecting only necessary columns of files
wf = wf [,-c(1:6)]
wy = wy [,-c(1:6)]
wa = wa [,-c(1:6)]
wg = wg [,-c(1:6)]
...
#log2-transformed intensity scores above 0 count as a positive peptide. Below 0 won't be considered a positive peptide
#so log2-transformed intensity scores > 0 gets value 1, log2-transformed intensity scores < 0 gets value 0
...
#plotting the density of W-substitutant distribution across samples
plot (density (rowSums(wf_c)), ylim=c(0,0.09), col="darkred", lwd=2)
lines (density (rowSums(wd_c)))
...
#thresholding to remain only peptides with higher specificity in expression. In this exemple, a cutoff of 25 has been used.
wy_c = wy_c [ rowSums (wy_c) < 25,]
wa_c = wa_c [ rowSums (wa_c) < 25,]
wg_c = wg_c [ rowSums (wg_c) < 25,]
wh_c = wh_c [ rowSums (wh_c) < 25,]
wc_c = wc_c [ rowSums (wc_c) < 25,]
wd_c = wd_c [ rowSums (wd_c) < 25,]
we_c = we_c [ rowSums (we_c) < 25,]
wi_c = wi_c [ rowSums (wi_c) < 25,]
wl_c = wl_c [ rowSums (wl_c) < 25,]
wm_c = wm_c [ rowSums (wm_c) < 25,]
wn_c = wn_c [ rowSums (wn_c) < 25,]
wp_c = wp_c [ rowSums (wp_c) < 25,]
wq_c = wq_c [ rowSums (wq_c) < 25,]
ws_c = ws_c [ rowSums (ws_c) < 25,]
wt_c = wt_c [ rowSums (wt_c) < 25,]
wv_c = wv_c [ rowSums (wv_c) < 25,]

#transforming data, sample id's become rownames and for each W-substitutant the value is the cumalative detections in the sample
...
#merging W-substitutions into one dataframe. Rownames are sample id's ('id') and columnnames are all W-substitutants (wf, wy, wa, etc)
#values are the number of detections of the W-substitutant in a sample. 
...
#barplotting cumalative W-substitutants in all samples
barplot (sort (colSums (counts [,-1])))

#tumor.txt contains list of CPTAC ids with tumor samples from PDC commons
#Same for normal.txt with CPTAC ids for adjacent normal tissue samples
...
tumor =merge (tumor, counts, by="id") #select only rows with tumor sample id's
normal =merge (normal, counts, by="id") #select only rows with adjacent normal tissue sample id's

#gene expression profiles from PDA samples reformatted to text file
exp = read.table ("T_EXP.txt", head=T)

#merge count dataframes with gene expression data from samples
